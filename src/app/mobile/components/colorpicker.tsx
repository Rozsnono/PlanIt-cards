export default function ColorPicker({ choosen }: { choosen: (color: string) => void }) {
    ...
    return (
        <main className="fixed w-full h-full flex justify-center items-center bg-[#00000060] z-[100]">
            ...
            <div className="w-[4rem] h-[4rem] flex justify-center items-center overflow-visible">
                <svg width="275" height="275" viewBox="0 0 275 275" className="overflow-visible">
                    {arcs.map((arc, index) => (
                        <path
                            onClick={() => { choosen(arc.name) }}
                            key={index}
                            d={describeArc(137.5, 137.5, 105.5, arc.startAngle, arc.endAngle)}
                            fill="none"
                            stroke={arc.color}
                            strokeWidth="50"
