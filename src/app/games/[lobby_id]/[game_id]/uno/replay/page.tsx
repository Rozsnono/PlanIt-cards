...
export default function Game() {
    ...
    return (
        <main className="flex w-full h-full rounded-md p-3 relative select-none">
            ...
            <main className="bg-red-800 rounded-md w-full relative flex justify-center items-center">
                ...
                <div className="flex gap-10  w-full absolute top-2 p-2 justify-center">
                    <div className="flex relative">
                        <Image className="absolute top-1 left-1" draggable={false} src={"/assets/cards/uno/Deck.png"} width={140} height={100} alt="card"></Image>
                        <Image draggable={false} className="border-2 border-transparent rounded-lg" src={"/assets/cards/uno/Deck.png"} width={140} height={110} alt="card"></Image>
                    </div>
                </div>

                <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-10 flex justify-center items-center">
                    <div className="flex relative" >
                        ...
                        }
                        {
                            ...
                            <Image className="right-1 bottom-1 rotate-12 border border-transparent rounded-lg" src={"/" + new CardsUrls().getFullCardUrl(getCurrentTurn()!.droppedCards[getCurrentTurn()!.droppedCards.length - 1].card.name)} width={140} height={100} alt="card"></Image>
                        }
                    </div>
                </div>
                ...
                <div className="absolute top-5 left-[5rem]">
                    <div className="flex items-center gap-1 text-xl text-red-300">
                ...
                <div className="absolute bottom-5 right-5">
                    <div className="flex items-center gap-1 text-xl text-red-300">
                        <Icon name="double-arrow-left" className={`cursor-pointer ${turn > 1 ? 'hover:animate-pulse ' : 'text-red-700'}`} size={48} onClick={() => {
                            if (turn > 1) {
                                setTurn(1);
                            }
                        }}></Icon>
                        <Icon name="arrow-left" className={`cursor-pointer ${turn > 1 ? 'hover:animate-pulse ' : 'text-red-700'}`} size={48} onClick={() => {
                            if (turn > 1) {
                                setTurn(turn - 1);
                            }
                        }}></Icon>
                        <Icon name="arrow-right" className={`cursor-pointer ${turn - 1 < Object.values(gameHistory.data.turns).length - 1 ? 'hover:animate-pulse ' : 'text-red-700'}`} size={48} onClick={() => {
                            if (turn - 1 < Object.values(gameHistory.data.turns).length - 1) {
                                setTurn(turn + 1);
                            }
                        }}></Icon>
                        <Icon name="double-arrow-right" className={`cursor-pointer ${turn - 1 < Object.values(gameHistory.data.turns).length - 1 ? 'hover:animate-pulse ' : 'text-red-700'}`} size={48} onClick={() => {
                            if (turn - 1 < Object.values(gameHistory.data.turns).length - 1) {
